...
export const authDirectiveTransformer = (schema: GraphQLSchema): GraphQLSchema => {
  return mapSchema(schema, {
    [MapperKind.OBJECT_FIELD]: (fieldConf) => {
      const authDirective = getDirective(schema, fieldConf, 'auth')
      if (authDirective) {
        const { resolve } = fieldConf
        fieldConf.resolve = async (parent, args, context, info) => {
          const request = {
