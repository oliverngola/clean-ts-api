...
describe('Login GraphQL', () => {
  ...
  describe('Login Query', () => {
    const query = `query {
      login(email: "ongola123@gmail.com", password: "123") {
        accessToken
        name
      }
    }`
    ...
    test('Should return an Account on valid credencials', async () => {
      ...
      expect(res.body.data.login.name).toBe('Oliveira Ngola')
    })
  ...
  describe('SignUp Mutation', () => {
    const query = `
      mutation {
        signUp(name: "Oliver", email: "ongola123@gmail.com", password: "123", passwordConfirmation: "123" ) {
          accessToken
          name
        }
      }
    ...
    test('Should return an Account on valid data', async () => {
      ...
      expect(res.body.data.signUp.name).toBe('Oliver')
    })
